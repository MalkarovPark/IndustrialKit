 ...
 */
public func perform_terminal_command(_ command: String, timeout: TimeInterval? = nil, output_handler: @escaping (String) -> Void = { _ in }) throws -> String
{
    let task = Process()
    let pipe = Pipe() // For both stdout and stderr

    task.standardOutput = pipe
    task.standardError = pipe // Combine stdout and stderr
    task.arguments = ["-c", command] // Execute command via shell
    // Using /bin/zsh as specified. Consider /bin/sh for wider compatibility
    // or if zsh-specifics are not needed.
    task.executableURL = URL(fileURLWithPath: "/bin/zsh")
    task.standardInput = nil // No input to the command

    let fileHandle = pipe.fileHandleForReading
    var accumulatedData = Data()
    // NSLock to protect access to accumulatedData from multiple threads
    // (readabilityHandler's thread and the calling thread)
    let dataLock = NSLock()

    // DispatchGroup to wait for task completion
    let group = DispatchGroup()

    // Set up the readability handler to process output as it comes
    fileHandle.readabilityHandler =
    { handle in
        let availableData = handle.availableData
        if availableData.isEmpty // End-of-file or pipe closed
        {
            // No more data will be available.
            // The task termination will be handled by task.waitUntilExit()
            return
        }

        dataLock.lock()
        accumulatedData.append(availableData)
        dataLock.unlock()

        if let outputChunk = String(data: availableData, encoding: .utf8)
        {
            // output_handler is called on a background thread.
            // If it needs to update UI, it must dispatch to DispatchQueue.main.
            output_handler(outputChunk)
        }
    }

    // Enter the group before starting the task monitoring
    group.enter()
    DispatchQueue.global().async // Perform blocking waitUntilExit on a background thread
    {
        task.waitUntilExit() // This blocks the current background thread, not the caller
        group.leave() // Signal that the task has completed
    }
    ...
    {
        // If task.run() itself throws (e.g., executable not found, permissions)
        fileHandle.readabilityHandler = nil // Clean up handler
        try? fileHandle.close() // Attempt to close file handle
        group.leave() // Ensure group is balanced if entered (though unlikely if run fails early)

        throw NSError(
            domain: "TerminalCommandError",
            code: -1, // Custom code for launch failure
            userInfo: [
                NSLocalizedDescriptionKey: "Failed to launch command '\(command)': \(error.localizedDescription)"
            ]
        )
    }
    ...
    {
        group.wait() // Wait indefinitely
        waitResult = .success
    }

    // Cleanup after waiting is complete, regardless of outcome
    // It's important to nil out the handler to stop further processing and break potential retain cycles.
    ...
    if waitResult == .timedOut
    {
        if task.isRunning
        {
            task.terminate() // Attempt to terminate the process
            // task.waitUntilExit() // Optionally wait a very short moment for termination to complete
        }
        try? fileHandle.close() // Close the pipe's reading end

        let finalOutputBeforeTimeout: String
        dataLock.lock()
        finalOutputBeforeTimeout = String(data: accumulatedData, encoding: .utf8) ?? "Undecodable output before timeout."
        dataLock.unlock()
        
        throw NSError(
            domain: "TerminalCommandError",
            code: Int(ETIMEDOUT), // POSIX error code for timeout
            userInfo: [
                NSLocalizedDescriptionKey: "Command '\(command)' timed out after \(timeout!) seconds. Output captured before timeout: \(finalOutputBeforeTimeout.prefix(200))..."
            ]
        )
    }

    // If we are here, the task completed without a timeout (waitResult == .success)
    // Ensure all remaining data from the pipe is read.
    // This is crucial as readabilityHandler might not have processed the very last chunk.
    // task.waitUntilExit() has completed, so the process is done writing.
    let remainingData = fileHandle.readDataToEndOfFile() // This blocks until EOF, safe now
    if !remainingData.isEmpty
    {
        dataLock.lock()
        accumulatedData.append(remainingData)
        dataLock.unlock()
        if let outputChunk = String(data: remainingData, encoding: .utf8)
        {
            output_handler(outputChunk)
        }
    }
    ...
    {
        dataLock.lock()
        let finalOutputString = String(data: accumulatedData, encoding: .utf8) ?? "Undecodable output."
        dataLock.unlock()
        ...
        {
            case .exit:
                reason = "exited with status \(task.terminationStatus)"
            case .uncaughtSignal:
                reason = "was terminated by signal \(task.terminationStatus)"
            @unknown default:
                reason = "terminated for an unknown reason with status \(task.terminationStatus)"
        }
        ...
        throw NSError(
            ...
            userInfo: [
                NSLocalizedDescriptionKey: "Command '\(command)' \(reason). Full output: \(finalOutputString.prefix(500))" // Limit output in error
            ]
        )
    }
    ...
    // Success
    dataLock.lock()
    let finalOutput = String(data: accumulatedData, encoding: .utf8) ?? ""
    dataLock.unlock()
